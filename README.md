...
| Kiosk doesn't cover the whole screen — GNOME top bar / dock still show | You're on a **Wayland** session (`echo $XDG_SESSION_TYPE`). The launcher passes `--ozone-platform-hint=auto` (native Wayland) + `--start-fullscreen` to handle this; if it still won't cover, log in on **Ubuntu on Xorg** (login screen → gear icon), or press **F11**. |
